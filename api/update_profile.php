...
$telefono = trim(strip_tags((string)($input['telefono'] ?? '')));
$direccion = trim(strip_tags((string)($input['direccion'] ?? '')));
...
// Transformar string vacío a NULL para que MySQL no lo considere un duplicado en la llave UNIQUE
$email_to_save = ($email === '') ? null : $email;

$sql = "UPDATE users SET email = ?, telefono = ?, direccion = ? WHERE id = ?";
$stmt = $conn->prepare($sql);

if (!$stmt) {
    echo json_encode(['success' => false, 'error' => 'Error interno al preparar la actualización.']);
    $conn->close();
    exit();
}

$stmt->bind_param("sssi", $email_to_save, $telefono, $direccion, $user_id);
