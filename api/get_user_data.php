...
if (isset($_SESSION['fresh_login']) && $_SESSION['fresh_login'] === true) {
    ...
    // Si el usuario fuerza la recarga, limitamos a 1 vez cada 15 segundos
...
// Ya NO escapamos variables manualmente porque usaremos Prepared Statements seguros
$identidad = trim($user_data_portal['identity_number']);
...
if (!$peticion) {
    ...
    if ($estadoCuenta_result && $estadoCuenta_result->num_rows > 0) {
        ...
        $financial_data = [
            ...
            'fecha_procesado' => $peticion['fecha_procesado'] ? date('d-m-Y H:i:s', strtotime($peticion['fecha_procesado'])) : null
        ];
    } else {
        $status_code = 'no_financial_records';
        $status_message = 'No se encontraron registros financieros.';
        $financial_data['fecha_procesado'] = $peticion['fecha_procesado'] ? date('d-m-Y H:i:s', strtotime($peticion['fecha_procesado'])) : null;
    }
    $stmt_estado->close();
}
